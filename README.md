# CustomAttributePotion

<br>

**CustomAttributePotion** — плагин для Minecraft, который добавляет настраиваемые зелья атрибутов.<br>
...
* Permission `cap.give` — выдача зелья игроку, обновить конфигурацию плагина.
* Permission `cap.use` — для использования рядовых команд `/cap check` .
* Permission `cap.list` — для просмотра всех доступных зелий `/cap list`.
...
|-----------------------------|-----------------------------------------------|------------|
| `/cap`                      | Основная команда плагина (показывает справку) | `cap.use`  |
| `/cap give <зелье> <игрок>` | Выдать зелье игроку                           | `cap.give` |
| `/cap check`                | Проверить активные зелья                      | `cap.use`  |
| `/cap list`                 | Список всех доступных зелий                   | `cap.list` |
| `/cap reload`               | Перезагрузить конфиг плагина                  | `cap.give` |
